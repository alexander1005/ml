...
def _loadData(filename):
    ...
    color = ['c', 'g', 'b', 'r']
    def _generateOne(X, Y, line):
        split = line.split(",")
        x, y, l = float(split[0]),float(split[1]),float(split[2])
        X.append((x, y))
        Y.append(l)
        return x, y
    for line in open(filename):
        # for _ in range(n_train):
        x_, y_ = _generateOne(X_train, Y_train, line)
        co = 0
        if y_ > 0.9:
            co = 1
        plt.scatter(x_, y_, s=100, c=color[co])
        # for _ in range(n_val):
        _generateOne(X_val, X_val, line)

    return np.array(X_train), np.array(X_val), np.array(Y_train), np.array(Y_val)

if __name__ == '__main__':
    model = SVMModel()
    X_t, X_v, Y_t, Y_v = _loadData('iris.csv')
    model.fit(X_t, Y_t)
